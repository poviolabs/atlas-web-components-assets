...
export const Tooltip = ({ children, text, ...tooltipProps }: TooltipProps) => {
  return (
    <AriaTooltipTrigger delay={0} closeDelay={0}>
      ...
      >
        <AriaOverlayArrow>
          <PointerHorizontalIcon className="h-2-5 w-5 text-interactive-secondary-idle group-placement-left:hidden group-placement-right:hidden group-placement-top:rotate-180" />
          <PointerVerticalIcon className="h-5 w-2-5 text-interactive-secondary-idle group-placement-left:rotate-180 group-placement-top:hidden group-placement-bottom:hidden" />
        </AriaOverlayArrow>
